...
title <- "Intro Model"
plot(model.intro$X, model.intro$Y, main = title)
...
Here it appears that we've lost our correlation entirely due to the line appearing with no slope in our plot. Let's take a closer look by plotting the model with the same scale as our previous model. We'll lose several of our Y outliers, but we'll get a closer look at the center of our Y values.
...
plot(model1$X, model1$Y, main = title, ylim = c(-10, 20))
```
...
Again, let's compare the Model 4 plot, this time with our Model 1 plot. While the residuals for both these plots utilize a normal distribution, however the Model 4 residuals have a heterogenous standard distribution produced by a process which generated residuals using a different standard deviation for multiple subsets of our 1000 simulated values.
...
```{r q6}
model1.estimate <- lm(Y~X, data = model1)
model1.summary <- summary(model1.estimate)
...
### 6a. ***Estimate the same parameters using the method of moments directly.***
