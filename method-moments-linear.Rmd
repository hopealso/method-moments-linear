...
Now we'll take the above simple introductory model and add a Gaussian residual representing variation in Y that is undescribed by our simplistic model above. Thus we will create X as our "independent" or "predictor"" random variable and Y as our "dependent" or "response" output random variable.
...
This is an important point to note, as it means that the standard deviation of Y is the same as the standard deviation of Epsilon ($\varepsilon$).

The equivalency of the two statements below points out two further important elements to note in our model.

$Y = \mu + \varepsilon$

$Y = aX + b + \varepsilon$

Firstly, we can observe that in our case "b" is equivalent to the mean or expected value of Y.
