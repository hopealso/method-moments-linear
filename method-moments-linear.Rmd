...
For this model the "true" answer to our question, at least the one which we coded into our pseudo-random Model 1 sample is the following:
...
Parameter  | True Value | `lm()` Estimate | MoM Estimate
-----------|------------|-----------------|--------------
a | `r a` | | `r model1.a.est`
...
$\rho^2$ | `r model2.rho.sq` | `r model1.summary$r.squared` | `r model2.rho.sq.est`
...
Y responds to predictor X in a linear fashion with some additional heteroscedastic randomness (residuals) which may in some ways appear Gaussian yet due to an inconsistent standard deviation in the data exhibits XXX ??? XXX behavior, according to the equation:
