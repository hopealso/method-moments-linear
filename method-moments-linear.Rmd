...
* _Files required: For Test: 'Week3_Test_Sample.csv'; store in RStudio project directory._
...
In this model, the value of the random variable Y varies, i.e. it is not a constant. A portion of this variation is *explained* by the values of X or is *predicted* by X (and the coefficients of our linear equation). This portion is described by our earlier introductory model:
...
plot(df$X, df$Y)
y.est <- df$X * coef(test.fit)[2] + coef(test.fit)[1]
plot(df$X, y.est)
...
boxplot(test.eps)
hist(test.eps)
plot(density(test.eps))
par(mfrow=c(1,1))

plot(df$X, df$Y, xlim = c(0, 5), ylim = c(0,10))
```
